...
<xsl:stylesheet version="1.0"
	...
	<xsl:template match="/CheckApplResp">
		...
		<RETURN>
			...
			<MAIN>
				<!-- 电子渠道:核保时保单号传交易流水号 -->
				<POLICY><xsl:value-of select="RespHead/SeqNo" /></POLICY>
				<!-- 电子渠道:投保单号传交易流水号 -->
				<APP/>
				...
				<xsl:variable name="temp_PREMC" select="RespBody/PolItem/PremAmt"/>
				<!-- 转为大写金额 -->
				<PREMC><xsl:value-of select="java:com.sinosoft.util.NumberUtil.getChnMoney($temp_PREMC)" /></PREMC>
				<PREM><xsl:value-of select="RespBody/PolItem/PremAmt" /></PREM>
				<VALIDATE><xsl:value-of select="java:com.sinosoft.util.DateUtil.date10To8(RespBody/PrdList/PrdItem[IsMain='1']/ValiDate)" /></VALIDATE>
				<INVALIDATE><xsl:value-of select="java:com.sinosoft.util.DateUtil.date10To8(RespBody/PrdList/PrdItem[IsMain='1']/InvaliDate)" /></INVALIDATE>
				<PAYDATECHN><xsl:value-of select="java:com.sinosoft.util.DateUtil.date10To8(RespHead/BusDate)" /></PAYDATECHN>
			...
			<PTS>
				<xsl:variable name="mainPrdId" select="RespBody/PrdList/PrdItem[IsMain='1']/PrdId" />
				<!-- 循环项 -->
				<xsl:for-each select="RespBody/PrdList/PrdItem">
					<PT>
						<PAYENDDATE><xsl:value-of select="java:com.sinosoft.util.DateUtil.date10To8(ExtItem/Ext010)" /></PAYENDDATE>
						<UNIT><xsl:value-of select="Units" /></UNIT>
						<AMT><xsl:value-of select="CovAmt" /></AMT>
						<PREM><xsl:value-of select="PayAmt" /></PREM>
						<ID><xsl:value-of select="PrdId" /></ID>
						<!-- TODO -->
						<MAINID><xsl:value-of select="$mainPrdId" /></MAINID>
						<NAME></NAME>
						<PERIOD></PERIOD>
						<COVERAGE_YEAR><xsl:value-of select="CovTerm" /></COVERAGE_YEAR>
						<COVERAGE_YEAR_DESC><xsl:value-of select="CovTerm" /></COVERAGE_YEAR_DESC>
						<PAYMETHOD>
							<xsl:call-template name="respTransPremType">
								<xsl:with-param name="PremType" select="PremType" />
							</xsl:call-template>
						</PAYMETHOD>
						<CHARGE_PERIOD>
							<xsl:call-template name="respTransPremTermType">
								<xsl:with-param name="PremTermType" select="PremTermType" />
							</xsl:call-template>
						</CHARGE_PERIOD>
						<CHARGE_YEAR><xsl:value-of select="PremTerm" /></CHARGE_YEAR>
						<PAYTODATE></PAYTODATE>
